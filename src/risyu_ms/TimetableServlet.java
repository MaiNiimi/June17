...
public class TimetableServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		String query = "select from " + Lesson2.class.getName() + " where className == " + className;
		// PrintWriter out = response.getWriter();
		List<Lesson2> list = null;
		try {
			list = (List<Lesson2>) manager.newQuery(query).execute();
			// http://puyooboe.blogspot.jp/2015/12/gae.html
		} catch (JDOObjectNotFoundException e) {
		}
		String res = "[";
		if (list != null) {
			for (Lesson2 data : list) {
